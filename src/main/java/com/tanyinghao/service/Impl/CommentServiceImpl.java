...
import com.tanyinghao.comm.enums.CommentTypeEnum;
...
import static com.tanyinghao.comm.constant.RedisConstant.COMMENT_LIKE_COUNT;
import static com.tanyinghao.comm.enums.CommentTypeEnum.ARTICLE;
...
public class CommentServiceImpl extends ServiceImpl<CommentMapper, Comment> implements CommentService {
    ...
    public void addComment(CommentDTO comment) {
        ...
        // 查询评论用户昵称
        String nickname = userMapper.selectOne(new LambdaQueryWrapper<User>()
                .select(User::getNickname).eq(User::getId, StpUtil.getLoginIdAsInt())).getNickname();
        // 通知用户
        if (siteConfig.getEmailNotice().equals(TRUE)) {
            CompletableFuture.runAsync( () -> {
                System.out.println("你好");
                notice(newComment, nickname);
            });
        }
    }
     ...
     * @Author TanYingHao
     * @Description 评论通知
     * @Date 18:36 2024/6/14
     * @Param [comment, nickname] 评论 昵称
     **/
    private void notice(Comment comment, String nickname) {
        ...
        if (StringUtils.hasText(toUser.getEmail())) {
            sendEmail(comment, toUser, title, nickname);
        }
    }
    /**
     *
     ...
     * @Param [comment, toUser, title, nickname]
     **/
     ...
     *
     * @Author TanYingHao
     * @Description 校验评论是否符合要求
     * @Date 16:57 2024/6/14
     * @Param [comment]
     **/
    private void verifyComment(CommentDTO comment) {
        ...
        if (comment.getCommentType().equals(ARTICLE.getType())) {
            Article article = articleMapper.selectOne(new LambdaQueryWrapper<Article>()
                    .select(Article::getId).eq(Article::getId, comment.getTypeId()));
        ...
        Optional.ofNullable(comment.getParentId()).ifPresent(parentId -> {
            ...
            Assert.isNull(parentComment.getParentId(), "当前评论是子评论，不能作为父评论");
            ...
            Comment replyComment = commentMapper.selectOne(new LambdaQueryWrapper<Comment>()
                    ...
                    .eq(Comment::getId, comment.getReplyId()));
            User toUser = userMapper.selectOne(new LambdaQueryWrapper<User>()
                    .select(User::getId).eq(User::getId, comment.getToUid()));
            Assert.notNull(replyComment, "回复的评论不存在");
            Assert.notNull(toUser, "回复的用户不存在");
            Assert.isTrue(comment.getCommentType().equals(replyComment.getCommentType()), "只能回复与同类型下的评论");
            ...
            Assert.isTrue(replyComment.getFromUid().equals(comment.getToUid()), "提交的评论toUid和当前回复评论的fromUid不一致");
            // 只能回复当前父评论及其子评论
